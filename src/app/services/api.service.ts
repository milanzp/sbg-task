...
export class ApiService {
    ...
    getTasksCollection(
        ...
        if (paginationOptions.index) {
            params = params.set(
                'offset',
                (paginationOptions.index * paginationOptions.size).toString()
            );
        }
        if (status) {
        ...
        return this.httpClient
            .get(`${API_URL}/tasks`, { params, headers, observe: 'response' })
            .pipe(
    ...
    getTaskDetails(id: string): Observable<TaskDetails> {
        return this.httpClient
            .get<TaskDetails>(`${API_URL}/tasks/${id}`, {
                headers
            })
            .pipe(map(TaskDetails.fromData));
    }

    getUserInfo(): Observable<User> {
        return this.httpClient
            .get<User>(`${API_URL}/user`, { headers })
            .pipe(map(User.fromData));
    }
